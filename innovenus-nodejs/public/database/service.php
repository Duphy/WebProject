...
function connect_to_server_and_send_message($msg, $socket = NULL){
	...
	if ($socket == NULL){
		...
		$fp = connect_to_server_and_return_fp();
		if ($fp==null)
			return null;
		$should_close_socket = TRUE;
	...
	/*foreach ($msg_array as $c)
		echo ' ' . ord($c);
*/
	fwrite($fp, $msg, strlen($msg)) or die("Could not write socket\n");
	...
	//die("read here");
	$length_string = fread($fp, NUM_OF_BYTES_IN_LENGTH) or die("Could not read length value in input\n");
	...
	// read
	$input = fread($fp, $length - NUM_OF_BYTES_IN_LENGTH) or die("Could not read content in input\n");
...
function connect_to_server_and_return_fp(){
