var net = require("net");
var host = "54.254.213.73";
var backup_host = "54.254.222.135";
var port = 1992;
...
function UserSocket() {
	...
	while ((self.pack_length != 0)
		...
		if ((header[3] == 6) && (header[4] == 0)) {
			...
			conns[dumb_session_key] = new UserSocket();
		    }
		}
		this.retry_count = 0;
	...
	console.log("socket ended, session_key: " + self.session_key + ", uid: " + self.uid);
	self.connected = false;
    });
    this.conn.on("close", function() {
	console.log("socket closed, session_key: " + self.session_key + ", uid: " + self.uid);
	self.connected = false;
    });
}
...
exports.disconnect = function(session_key) {
    console.log("disconnect socket, session_key: ", session_key);
    if (typeof conns[session_key] != "undefined") {
...
exports.localizedTime = function(date, time) {
    ...
    return [
	    print_date(d.getFullYear() * 10000 + (d.getMonth() + 1) * 100
		    + d.getDate()),
	    print_time(d.getHours() * 10000 + d.getMinutes() * 100
		    + d.getSeconds()) ];
